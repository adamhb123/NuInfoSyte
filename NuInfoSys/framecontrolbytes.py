...
class FrameControlBytes(enum.Enum):
    ...
    FILE_PRIORITY = b"0"
    FILE_NORMAL_RANGE = [x.to_bytes(1, "big") for x in range(0x20, 0x7F) if x != int.from_bytes(b"0", 'big')]
    ...
    NEWLINE = CR = b"\x0d"  # Carriage Return (new line)
    ...
    def __add__(self, other) -> bytes:
        if type(other) == FrameControlBytes:
