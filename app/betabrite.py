...
########################################################################
#                                                                      #
#   This project is a remake of the existing InfoSys project, with     #
#   the goal of being easier to read, maintain, and use.               #
#                                                                      #
#   This code is moderately based on Jonathan Koren's betabrite.py     #
#   script, so credit goes to him for a good portion of this. Most     #
#   of this was written fairly hastily so it is not the nicest code,   #
#   but she gets the job done, and I'll try and make it nicer over     #
#   time.                                                              #
#                                                                      #
########################################################################
...
ANIMATION_POS_DICT = {
    'middle': TEXT_POS_MIDDLE, 'top': TEXT_POS_TOP, 'bottom':TEXT_POS_BOTTOM,
    'fill': TEXT_POS_FILL}

ANIMATION_COLOR_DICT = {
    'red': TEXT_COLOR_RED, 'green': TEXT_COLOR_GREEN, 'amber': TEXT_COLOR_AMBER,
    'dimred': TEXT_COLOR_DIMRED, 'brown':TEXT_COLOR_BROWN,'orange': TEXT_COLOR_ORANGE,
    'yellow': TEXT_COLOR_YELLOW, 'rainbow1': TEXT_COLOR_RAINBOW1, 'rainbow2': TEXT_COLOR_RAINBOW2,
    'mix': TEXT_COLOR_MIX, 'autocolor': TEXT_COLOR_AUTO}

ANIMATION_MODE_DICT = {'rotate': MODE_ROTATE, 'hold': MODE_HOLD, 'flash': MODE_FLASH,
    'rollup': MODE_ROLLUP, 'rolldown': MODE_ROLLDOWN, 'rollleft': MODE_ROLLLEFT,
    'rollright': MODE_ROLLRIGHT, 'wipeup': MODE_WIPEUP, 'wipedown': MODE_WIPEDOWN,
    'wipeleft': MODE_WIPELEFT,'wiperight': MODE_WIPERIGHT, 'scroll': MODE_SCROLL,
    'automode': MODE_AUTO, 'rollin': MODE_ROLLIN, 'rollout': MODE_ROLLOUT,
    'wipein': MODE_WIPEIN, 'wipeout': MODE_WIPEOUT, 'cmprsrot': MODE_CMPRSROT,
    'twinkle': MODE_TWINKLE, 'sparkle': MODE_SPARKLE, 'snow': MODE_SNOW,
    'interlock': MODE_INTERLOCK, 'switch': MODE_SWITCH, 'spray': MODE_SPRAY,
    'starburst': MODE_STARBURST, 'welcome': MODE_WELCOME, 'slotmachine': MODE_SLOTMACHINE,
    'newsflash': MODE_NEWSFLASH, 'trumpet': MODE_TRUMPET, 'thankyou': MODE_THANKYOU,
    'nosmoking': MODE_NOSMOKING,'drinkdrive': MODE_DRINKDRIVE, 'animal':MODE_ANIMAL, 'fish':MODE_FISH,
    'fireworks': MODE_FIREWORKS, 'turbocar': MODE_TURBOCAR, 'balloons': MODE_BALLOONS, 'cherrybomb': MODE_CHERRYBOMB}

DOTS_TEST_ARROW = b"00000080000\r00000088000\r08888888800\r08888888880\r08888888800\r00000088000\r00000080000\r"

''' Configurables '''
# For use with direct cli interaction, as an animation separator
TERMINAL_AND = "-"
PORT = '/dev/serial/by-id/usb-Prolific_Technology_Inc._USB-Serial_Controller_D-if00-port0'


def _transmit(port, payload, addr=SIGN_ADDRESS_BROADCAST, type=SIGN_TYPE_ALL_VERIFY):
    packet = WAKEUP + SOH + type + addr + STX + payload + EOT
    ser = serial.Serial(port, 9600, timeout=10)
    ser.write(packet)
    ser.close()

def _write_file(animations:Union[List[Animation],Animation], file=FILE_PRIORITY):
    #   Many animations
    #   print(type(animations))
...
def cli_parse_animations_from_string(animation_string) -> List[Animation]:
    """
    animation_string should be formatted as such: 
    TEXT ANIMATION_MODE ANIMATION_COLOR ANIMATION_POSITION$TERMINAL_AND$NEXT_ANIMATION
    e.g. chungus cherrybomb rainbow2 None-bingus None amber None
    where '-' is replaced with the TERMINAL_AND
    """
    strlist = []
    for x in animation_string.split(TERMINAL_AND): strlist.append(x)
    return _parse_animations(strlist)

def cli_parse_animations(animations: List[str]):
    text, mode, color, pos = '', MODE_AUTO, TEXT_COLOR_AUTO, TEXT_POS_MIDDLE
    parsed_animations = []
    while len(animations) != 0:
        animget = animations.pop(0).split(',')
...
class Animation():
    ...
    def __init__(self, text="", mode=MODE_AUTO, color=TEXT_COLOR_AUTO, position=TEXT_POS_MIDDLE):
        self._text = text
        self._mode = ANIMATION_MODE_DICT[mode] if isinstance(mode, str) and mode in ANIMATION_MODE_DICT.keys() else (mode if mode in ANIMATION_MODE_DICT.values() else MODE_AUTO)
        self._color =  ANIMATION_COLOR_DICT[color] if isinstance(color, str) and color in ANIMATION_COLOR_DICT.keys() else (color if color in ANIMATION_COLOR_DICT.values() else TEXT_COLOR_AUTO)
        self._position = ANIMATION_POS_DICT[position] if isinstance(position, str) and position in ANIMATION_POS_DICT.keys() else (position if position in ANIMATION_POS_DICT.values() else TEXT_POS_MIDDLE)

    def __str__(self):
        return f"Animation: text='{self._text}' mode={self._mode} color={self._color} position={self._position}"
    ...
    def reset(self):
        ...
        self._color=TEXT_COLOR_AUTO
        self._position=TEXT_POS_MIDDLE

    def randomize(self):
        self._mode = random.choice(ANIMATION_MODE_DICT.values())
        self._color = random.choice(ANIMATION_COLOR_DICT.values())
        self._position = random.choice(ANIMATION_POS_DICT.values())

    def display(self):
        _transmit(PORT, _write_file(self))

    def bytestr(self):
        return SOM + self._position + self._mode + self._color + _transcode(self._text)


##### Not even close to working #####
def display_DOTS(dots_data, port=PORT):
    #   Setup DOTS
    _transmit(port, COMMAND_WRITE_SPECIAL + b"\x24" + b"JDU" + b"\x07\x0B"+ b"4000")
    _transmit(port, COMMAND_WRITE_DOTS+b"J"+b"\x07\x0B"+DOTS_TEST_ARROW)
    print(COMMAND_WRITE_SPECIAL + b"\x24" + b"JDU" + b"\x07\x0B"+ b"4000")
    print(COMMAND_WRITE_DOTS+b"J"+b"\x07\x0B"+DOTS_TEST_ARROW)
    #  Setup TEXT
    _transmit(port, COMMAND_WRITE_SPECIAL + b"\x24" + b"H" + b"AU" + b"\x00\x2C"+b"\xFF\xFE")
    _transmit(port, COMMAND_WRITE_TEXT + b"H" + b"\x1b\x20"+ b"\x62" + b"ffffffffffffffffffffffffffffffffffffffffffff")

    #   Set run sequence
    _transmit(port, COMMAND_WRITE_SPECIAL + b"\x2eSUH")

def soft_reset(port=PORT):
    _transmit(port, COMMAND_WRITE_SPECIAL + b"\x2c")

def send_animations(animations: List[Animation]):
    #   If you want to send just one animation, use its 'display()' method
    _transmit(PORT, _write_file(animations))
...
def main():
    ...
    animations = cli_parse_animations_from_string(animations)
    print(animations)
    #_transmit(PORT, _write_file(animations))
